...
- `BattleState.fleeIn: number | null` を足す。宣言時に `rng.int(1, 3)` を入れる
- `endTurn` の敵の行動が終わったあとに 1 減らし、0 になったら
  `outcome = 'fled'` にする (`BattleOutcome` に `'fled'` を足す)
- `game.ts` は `fled` を勝利でも敗北でもなく扱う。パーティ HP は引き継ぎ、
  報酬は無し、遭遇は消費 (`run.pending` は既に null なのでそのまま探索へ戻る)。
  ボス戦から逃げた場合も同じ (区画は解放されない)
- Action `{ type: 'battle-flee' }` はトグル。`fleeIn` が null なら宣言、
  非 null なら null に戻す (キャンセル)
- `BattleView.fleeIn` を足し、上部ステータス欄に残りターンを出す

## 8. 操作クラスタの再配置 (UI)

- 操作列をキャラウィンドウと同じ高さに揃え、メッセージ欄に食い込ませて縦を稼ぐ
- 常用群 (ターン終了 / 防御) を大きく上に、臨時群 (回復薬 / 編成 / 逃げる) を
  小さく下にまとめる
- 防御の枚数と逃走までの残りターンは上部のステータス欄に移す。
  ボタンのラベルからは数字を外す
